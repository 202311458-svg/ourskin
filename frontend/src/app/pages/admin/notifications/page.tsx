...
export default function AdminNotifications() {
  ...
  const markRead = async (item: NotificationItem, navigate = false) => {
    ...
    try {
      ...
      if (navigate && item.target_url?.startsWith("/pages/")) { window.location.assign(item.target_url); return; }
      await load();
  ...
  const markAllRead = async () => {
    setFeedback(""); setError("");
    try { const result = await notificationApi.markAllRead(); setFeedback(`${result.updated_count} notification${result.updated_count === 1 ? "" : "s"} marked as read.`); await load(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to update notifications."); }
  };

  return (
    <PageShell>
      <PageHeader eyebrow="Communication center" title="Notifications" description="Review administrative, appointment, and system notifications sent to your account." primaryAction={<AdminActionButton tone="secondary" onClick={() => void markAllRead()} disabled={(summary?.unread ?? 0) === 0}>Mark all as read</AdminActionButton>} />
      ...
      <AdminToolbar meta={`${total} matching notification${total === 1 ? "" : "s"}`}>
        <input type="search" aria-label="Search notifications" placeholder="Search title, message, or type" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
        <select value={readState} onChange={(e) => { setReadState(e.target.value); setPage(1); }} aria-label="Filter read state"><option value="all">All states</option><option value="unread">Unread</option><option value="read">Read</option></select>
        <select value={notificationType} onChange={(e) => { setNotificationType(e.target.value); setPage(1); }} aria-label="Filter notification type"><option value="all">All types</option>{typeOptions.map((type) => <option key={type} value={type}>{pretty(type)}</option>)}</select>
      </AdminToolbar>
      <AdminDataTable title="Inbox" description="Notifications are private to your administrator account. Opening an internal target marks the item read first." loading={loading} loadingText="Loading notifications…" error={error} empty={!loading && !error && items.length === 0} emptyTitle="No notifications match this view." emptyDescription="Try changing the search or filters.">
        <table><thead><tr><th>Notification</th><th>Type</th><th>Status</th><th>Received</th><th>Related record</th><th>Actions</th></tr></thead>
        <tbody>{items.map((item) => <tr key={item.id} className={!item.is_read ? styles.unreadRow : undefined}><td className={styles.titleCell}><strong>{item.title}</strong><span>{item.message}</span></td><td><StatusBadge tone="info">{pretty(item.notification_type)}</StatusBadge></td><td><StatusBadge tone={item.is_read ? "success" : "warning"}>{item.is_read ? "Read" : "Unread"}</StatusBadge></td><td>{formatDate(item.created_at)}</td><td>{item.related_entity_type ? `${pretty(item.related_entity_type)} ${item.related_entity_id || ""}`.trim() : "N/A"}</td><td><div className={styles.actions}>{!item.is_read ? <AdminActionButton onClick={() => void markRead(item)}>Mark read</AdminActionButton> : null}{item.target_url?.startsWith("/pages/") ? <AdminActionButton tone="primary" onClick={() => void markRead(item, true)}>Open</AdminActionButton> : null}</div></td></tr>)}</tbody></table>
      </AdminDataTable>
