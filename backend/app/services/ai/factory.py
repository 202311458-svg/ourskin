...
from app.services.ai.providers.openai_progress_provider import (
    OpenAIProgressAnalysisProvider,
)
...
def _openai_key(settings: Settings) -> str:
    ...
    if settings.openai_api_key is None:
        raise RuntimeError(
            "OPENAI_API_KEY is required before the AI pipeline can run"
        )
    return settings.openai_api_key.get_secret_value()


def build_dermatology_analysis_service(
    settings: Settings,
) -> DermatologyAnalysisService:
